...
| `motion/reveal.tsx` | Shared Motion scroll-reveal primitives (`"use client"`): `Reveal` (single element fade+rise on `whileInView`, `once`), `RevealGroup` + `RevealItem` (staggered list via variants). All honour `prefers-reduced-motion` (collapse to fade, no movement). Replaces the old per-section IntersectionObserver + Tailwind reveal pattern (currently used by `services-section`/`process-section`). |
